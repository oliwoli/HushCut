        ...
        <>
            <div className="flex items-center h-64 select-none">
                <SliderPrimitive.Root
                    ...
                    className="relative flex touch-none select-none data-[orientation=vertical]:h-full data-[orientation=vertical]:w-8 z-10" // Adjusted Root width to accommodate wider thumb
                >
                    <SliderPrimitive.Track className="bg-none relative grow rounded-none w-1.5">
                        <SliderPrimitive.Range className="bg-none absolute w-full rounded-none" /> {/* Added rounded-2xl to match track */}
                    </SliderPrimitive.Track>
                    <SliderPrimitive.Thumb
                        className={cn(
                            "block h-6 w-8 bg-zinc-300 rounded-xs relative shadow-xs shadow-zinc-950", // h-6 for desired height, w-8 to extend
                    ...
                    >
                        ...
                        <div
                            className="absolute top-1/2 -translate-y-1/2 h-[2px] w-4 bg-zinc-800 pointer-events-none left-1/2 transform -translate-x-1/2"
                        />
                    </SliderPrimitive.Thumb>
                </SliderPrimitive.Root>

                <div className="relative h-58 bg-zinc-500 w-1 z-0 right-[18px] rounded-xs"></div>
                <div className="relative h-[99%] ml-2 top-4">
                    {marks.map((dB) => {
                        const g = db2gain(dB);
