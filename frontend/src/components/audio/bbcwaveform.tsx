...
import { SilencePeriod } from "@/types";
...
const normalizeWaveformData = (
  ...
  console.log(
    `Normalizing data with ${data.length} points, bits: ${inputData.bits}`
  );
  ...
  console.log(
    `Normalization: Scaling factor: ${scalingFactor} (targetPeak: ${targetPeak})`
  );
...
const WaveformPlayer: React.FC<WaveformPlayerProps> = ({
  ...
  const [originalWaveformData, setOriginalWaveformData] =
    useState<JsonWaveformData | null>(null);
  const [processedWaveformData, setProcessedWaveformData] =
    useState<JsonWaveformData | null>(null);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      let normalizedDataForPeaks: JsonWaveformData | null = null; // To hold the data for Peaks.js
      ...
      try {
        ...
        setOriginalWaveformData(fetchedData); // Store the original data

        const normalized = normalizeWaveformData(fetchedData); // Normalize it
        setProcessedWaveformData(normalized); // Store the processed data
        console.log("Waveform data processed (normalized):", normalized);
        normalizedDataForPeaks = normalized || fetchedData;
        ...
        const options: PeaksOptions = {
          zoomview: {
            ...
            segmentOptions: {
              overlayColor: "#00ff44",
              overlayBorderColor: "#00ff3c",
              overlayBorderWidth: 2,
              waveformColor: "#00ff44",
              markers: true,
              endMarkerColor: "#00ff3c",
              startMarkerColor: "#c300ff",
              overlay: false,
              overlayOpacity: 0.5,
              overlayOffset: 10,
          ...
          overview: {
            ...
            enableSegments: true,
            segmentOptions: {
              overlayColor: "rgba(0, 255, 72, 0.435)",
              overlayBorderColor: "#00ff3c",
              overlayBorderWidth: 2,
              waveformColor: "#00ff44",
              overlay: true,
          ...
          waveformData: { json: normalizedDataForPeaks },
        };
        ...
        await new Promise<void>((resolve, reject) => {
          Peaks.init(options, (err, peaksInstanceCallback) => {
            ...
            // ... (handle err, peaksInstanceCallback, setPeaksInstance, resolve/reject -
            if (err) {
    ...
    return () => {
      ...
      // ... (rest of cleanup: destroy peaks, disconnect nodes, cancel animation) ...
  ...
  useEffect(() => {
    // Ensure all required data and refs are available
    if (
      !peaksInstance || // Ensures peaks.js is ready (and thus audioEl duration might be available)
      !processedWaveformData ||
      !processedWaveformData.data ||
      processedWaveformData.data.length === 0 ||
      !audioElementRef.current ||
      !peakMeterRef.current
    ) {
      // Set peak meter to 0 if prerequisites are not met
    ...
    const audioEl = audioElementRef.current;
    const waveformDataPoints = processedWaveformData.data;
    const dataLength = waveformDataPoints.length;
    const bits = processedWaveformData.bits === 8 ? 8 : 16;
    const maxAbsWaveformValue = bits === 8 ? 127 : 32767; // Max value after your normalization

    let animationFrameId: number | null = null;

    const updatePeakMeter = () => {
      // Ensure audioEl.duration is valid and not 0 to prevent division by zero
      if (!audioEl.duration || !isPlaying) {
        // Only update if playing
        if (peakMeterRef.current && audioEl.currentTime === 0) {
          // Reset if paused at start
          peakMeterRef.current.style.transform = `scaleY(0)`;
        }
        animationFrameId = requestAnimationFrame(updatePeakMeter); // Keep checking if it starts playing
        return;
      }
      ...
      const dataIndex = Math.min(
        Math.max(0, Math.floor((currentTime / duration) * dataLength)),
        dataLength - 1
      );

      const currentPeakValue = Math.abs(waveformDataPoints[dataIndex]);
      const normalizedDisplayPeak = currentPeakValue / maxAbsWaveformValue;

      if (peakMeterRef.current) {
        peakMeterRef.current.style.transform = `scaleY(${normalizedDisplayPeak})`;
      }
    ...
    return () => {
      ...
      // Optionally reset peak meter on cleanup
      if (peakMeterRef.current) {
        peakMeterRef.current.style.transform = `scaleY(0)`;
      }
    };
  ...
  useEffect(() => {
    ...
    const allCurrentSegments = peaksInstance.segments.getSegments();
    let segmentsActuallyRemovedCount = 0;
    allCurrentSegments.forEach((seg) => {
      if (seg.id && seg.id.startsWith("silence-marker-")) {
        try {
          peaksInstance.segments.removeById(seg.id);
          segmentsActuallyRemovedCount++;
        } catch (e) {
          console.warn(`Could not remove old segment with ID ${seg.id}:`, e);
        }
      }
    });

    const batchId = Date.now(); // Creates a unique prefix for this batch of segments
    const newSilenceSegmentOptions: SegmentOptions[] = []; // Explicitly SegmentOptions[]

    if (silenceData && silenceData.length > 0) {
      silenceData.forEach((period, index) => {
        newSilenceSegmentOptions.push({
          startTime: period.start,
          endTime: period.end,
          //labelText: `Silence ${index + 1}`,
          // ID includes the batchId to ensure it's unique across effect runs
          id: `silence-marker-${batchId}-${index}-${period.start.toFixed(
            3
          )}-${period.end.toFixed(3)}`,
          editable: false,
          color: "rgb(255, 0, 111)",
        });
      });
    ...
    // 3. Add the new batch of segments if there are any.
    if (newSilenceSegmentOptions.length > 0) {
      try {
        peaksInstance.segments.add(newSilenceSegmentOptions);
      } catch (e: any) {
        // This error should ideally not be "duplicate id" anymore with this strategy.
        // If it is, something more fundamental is wrong with peaks.js state or the IDs.
        console.error(
          "Segment effect: CRITICAL Error during peaks.segments.add() with unique batch IDs:",
          e.message,
          e
        );
        console.error(
          "Segment effect: Segments it tried to add:",
          JSON.stringify(newSilenceSegmentOptions.map((s) => s.id))
        );
        // Log current segments to see what's there
        console.error(
          "Segment effect: Segments in instance at time of error:",
          JSON.stringify(peaksInstance.segments.getSegments().map((s) => s.id))
        );
      }
    }
  }, [peaksInstance, silenceData]);
